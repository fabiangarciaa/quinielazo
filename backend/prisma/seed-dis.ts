...
async function main() {
  console.log('Seed DIS — Mundial 2026\n');

  // Verificar que el torneo existe
  const tournament = await prisma.tournament.findUnique({ where: { id: TOURNAMENT_ID } });
  if (!tournament) { console.error('Torneo no encontrado'); process.exit(1); }
  console.log(`✅ Torneo: ${tournament.name}`);

  // Limpiar datos existentes del torneo (sin borrar participantes)
  ...
  await prisma.pot.deleteMany({ where: { tournamentId: TOURNAMENT_ID } });
  await prisma.tournamentPhase.deleteMany({ where: { tournamentId: TOURNAMENT_ID } });
  console.log('Limpio\n');

  // Crear fases
  const faseGrupos = await prisma.tournamentPhase.create({
    data: { tournamentId: TOURNAMENT_ID, name: 'Fase de grupos', type: 'GROUP_STAGE', roundNumber: 1, isActive: true }
  });
  for (const [name, type, round] of [
    ...
    await prisma.tournamentPhase.create({
      data: { tournamentId: TOURNAMENT_ID, name, type: type as any, roundNumber: round, isActive: false }
    });
  }
  console.log('✅ 7 fases creadas');
  ...
  for (let i = 0; i < 8; i++) {
    ...
    bombos.push(await prisma.pot.create({
      data: {
        tournamentId: TOURNAMENT_ID,
        name: BOMBO_NOMBRES[i],
        level: i + 1,
        strengthMin: Math.min(...s),
        strengthMax: Math.max(...s),

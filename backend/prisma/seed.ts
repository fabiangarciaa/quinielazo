...
const SCORING_RULES_MUNDIAL = [
  ...
  { eventType: 'ADVANCE_ROUND_OF_32' as ScoringEventType, points: 5, description: 'Clasificar a 16avos de final' },
...
const SCORING_RULES_LIGAMX = [
  { eventType: 'WIN_GROUP' as ScoringEventType, points: 3, description: 'Victoria en fase regular' },
  { eventType: 'DRAW_GROUP' as ScoringEventType, points: 1, description: 'Empate en fase regular' },
  { eventType: 'ADVANCE_ROUND_OF_32' as ScoringEventType, points: 2, description: 'Clasificar a Play-In' },
  { eventType: 'ADVANCE_ROUND_OF_16' as ScoringEventType, points: 5, description: 'Clasificar a Liguilla' },
  { eventType: 'ADVANCE_SEMI' as ScoringEventType, points: 6, description: 'Avanzar a semifinal' },
  { eventType: 'REACH_FINAL' as ScoringEventType, points: 8, description: 'Avanzar a final' },
  { eventType: 'CHAMPION' as ScoringEventType, points: 15, description: 'Campeón del torneo' },
  { eventType: 'CLEAN_SHEET' as ScoringEventType, points: 1, description: 'Portería en cero' },
  { eventType: 'THRASHING_WIN' as ScoringEventType, points: 2, description: 'Goleada por 3+ goles' },
  { eventType: 'SUPER_LEADERSHIP' as ScoringEventType, points: 5, description: 'Superliderato' },
];
...
function et(iso: string): Date { return new Date(iso + '-04:00'); }
...
async function main() {
  ...
  for (const r of SCORING_RULES_MUNDIAL) await prisma.scoringRule.create({ data: { ...r, tournamentId: mundial.id } });
  ...
  for (const [name, type, round] of [['16avos de final','ROUND_OF_32',2],['Octavos de final','ROUND_OF_16',3],['Cuartos de final','QUARTER_FINAL',4],['Semifinal','SEMI_FINAL',5],['Tercer lugar','THIRD_PLACE',6],['Final','FINAL',7]] as [string,string,number][]) {
    await prisma.tournamentPhase.create({ data: { tournamentId: mundial.id, name, type: type as any, roundNumber: round, isActive: false } });
  }
  ...
  for (const [local, visitante, fechaET, sede, grupo] of PARTIDOS) {
    ...
    if (!homeId || !awayId) { console.warn(`No encontrado: ${local} vs ${visitante}`); continue; }
  ...
  const ligaMx = await prisma.tournament.create({ data: { name: 'Liga MX — Apertura 2026', type: TournamentType.LIGA_MX, season: 'Apertura 2026', teamCount: 18, participantCount: 6, competitionSystem: 'Fase regular + Play-In + Liguilla', status: TournamentStatus.SETUP, scoringConfig: {} } });
  for (const t of LIGA_MX_TEAMS) await prisma.team.create({ data: { ...t, tournamentId: ligaMx.id } });
  for (const r of SCORING_RULES_LIGAMX) await prisma.scoringRule.create({ data: { ...r, tournamentId: ligaMx.id } });
  for (const [name, type, round, active] of [['Fase regular','REGULAR_SEASON',1,true],['Play-In','PLAY_IN',2,false],['Liguilla','LIGUILLA',3,false],['Cuartos','QUARTER_FINAL',4,false],['Semifinal','SEMI_FINAL',5,false],['Final','FINAL',6,false]] as [string,string,number,boolean][]) {
    await prisma.tournamentPhase.create({ data: { tournamentId: ligaMx.id, name, type: type as any, roundNumber: round, isActive: active } });
  }

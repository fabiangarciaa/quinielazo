...
import { Users, Shield, Shuffle, Calendar, BarChart2, Zap, ChevronRight, Settings, Table2 } from 'lucide-react';
...
export function TournamentDetailPage() {
  ...
  // Links visibles para ADMIN
  const adminLinks = [
    { to: 'participants', icon: Users, label: 'Participantes', count: t._count?.participants ?? t.participants?.length ?? 0, color: 'blue' },
  ...
  const commonLinks = [
    { to: 'groups', icon: Table2, label: 'Grupos', color: 'teal' },
    { to: 'matches', icon: Calendar, label: 'Partidos', count: t._count?.matches || 0, color: 'orange' },
    { to: 'ranking', icon: BarChart2, label: 'Ranking', color: 'amber' },
    { to: 'simulator', icon: Zap, label: 'Simulador', color: 'pink' },
  ];

  const quickLinks = isAdmin ? [...adminLinks, ...commonLinks] : commonLinks;
  ...
  return (
    <div className="p-3 md:p-6 max-w-5xl mx-auto space-y-4 md:space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-4 md:p-6">
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 min-w-0">
            <p className="text-xs text-gray-400 uppercase tracking-wider">{t.type.replace('_', ' ')} · {t.season}</p>
            <h1 className="text-xl md:text-2xl font-bold text-gray-800 mt-0.5">{t.name}</h1>
            ...
            <div className="flex items-center gap-2 mt-3 flex-wrap">
              ...
              {isAdmin && <Stat label="Partidos" value={t._count?.matches || 0} />}
            </div>
          </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 md:gap-3">
        {quickLinks.map(({ to, icon: Icon, label, count, color }: any) => (
          <Link key={to} to={to}
            className="bg-white rounded-xl border border-gray-100 p-3 md:p-4 hover:shadow-md transition-all hover:border-gray-200 flex items-center gap-2 md:gap-3 group">
            <div className={clsx('w-9 h-9 md:w-10 md:h-10 rounded-xl flex items-center justify-center shrink-0', colorMap[color])}>
              <Icon size={18} />
            </div>
            <div className="flex-1 min-w-0">
      ...
      {t.participants?.length > 0 && (
        <div className="bg-white rounded-2xl border border-gray-100 p-4 md:p-5">
          <h2 className="font-semibold text-gray-800 mb-3 md:mb-4">Participantes y equipos</h2>
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={clsx('text-xs font-semibold px-3 py-1 rounded-full', map[status])}>
      {labels[status]}
    </span>
  );
}